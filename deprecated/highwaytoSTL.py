...
def shapefile_highways_to_stl_aligned(shapefile_path, output_path,
    ...
    """
    Convert highway shapefile to STL using combined offset for alignment
    ...
    # Use the provided combined offset
    ...
    for idx, row in line_gdf.iterrows():
        ...
        for line in lines:
            try:
                ...
                if use_local_coords:
                    adjusted_coords = [(x - combined_offset_x, y - combined_offset_y) for x, y in coords]
                else:
...
# Main execution - Highway STL only
if __name__ == "__main__":
    ...
    # Step 1: Calculate combined offset from all shapefiles for alignment
    print("🔄 Calculating combined offset for alignment...")
    shapefiles = []
    
    # Add existing shapefiles for offset calculation
    ...
    if not shapefiles:
        print("❌ No shapefiles found! Please check paths.")
        exit(1)
    
    offset_x, offset_y = get_combined_offset(shapefiles, target_epsg=32616)
    
    # Step 2: Generate highways only
    if os.path.exists(highway_shp):
        print("\n🛣️ Generating highway STL with combined alignment...")
        ...
        highway_result = shapefile_highways_to_stl_aligned(
            ...
            combined_offset_x=offset_x,
            combined_offset_y=offset_y,
        ...
        print(f"\n✅ Highway STL generated and aligned using combined offset: ({offset_x:.2f}, {offset_y:.2f})")
