#!/usr/bin/env python3
"""
Rectangular Base STL Generator with Terrain Integration
Creates a rectangular base/platform that uses terrain as the top surface,
removing any parts above the terrain for a perfect fit.
"""
import numpy as np
import struct
import os
...
class RectangularBaseGenerator:
    ...
    def load_terrain_from_stl(self, stl_filename: str) -> bool:
        ...
        try:
            if stl_filename.lower().endswith('.stl'):
                return self._load_stl_binary(stl_filename)
            else:
    ...
    def _get_terrain_height_at_point(self, x: float, y: float) -> Optional[float]:
        """Get terrain height at a specific X,Y coordinate using triangle interpolation"""
        best_z = None
    ...
    def create_terrain_fitted_base(self, base_bounds: Dict[str, float], base_thickness: float = 20.0,
        ...
        print(f"  Points with terrain data: {points_with_terrain}/{len(x_range) * len(y_range)}")
    ...
    def write_stl_binary(self, filename: str) -> bool:
        ...
        try:
            with open(filename, 'wb') as f:
                ...
                header = b'Terrain-fitted base STL' + b'\0' * (80 - 23)
                f.write(header)
    ...
    def write_stl_ascii(self, filename: str) -> bool:
        ...
        try:
            with open(filename, 'w') as f:
                f.write("solid terrain_fitted_base\n")
                ...
                f.write("endsolid terrain_fitted_base\n")
...
def main():
    ...
    GRID_RESOLUTION = 5.0           # Grid resolution for sampling terrain (smaller = more detail)
    
    # Standard rectangular base settings (fallback)
    BASE_EXTENTS = [-400, -350, 560, 400, 350, 580]
    
    # Output settings
    OUTPUT_FILE = "windAroundBuildings/Tools/output/wRaster/basewTerrain.stl"
    OUTPUT_ASCII = False
