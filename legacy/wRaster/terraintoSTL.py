...
def load_elevation_raster(tif_path, target_crs='EPSG:32616', resample_factor=1.0):
    ...
    try:
        return _load_with_rasterio(tif_path, target_crs, resample_factor)
    except Exception as e:
        print(f"Rasterio failed: {e}")
        print("Trying alternative methods...")
        return _load_dem_data_alternative(tif_path, target_crs, resample_factor)

def _load_with_rasterio(tif_path, target_crs, resample_factor):
...
def _load_dem_data_alternative(tif_path, target_crs, resample_factor):
...
def add_base_and_sides(triangles, base_thickness=None):
    """Add base and side walls to create a solid terrain block."""
    print("Adding base and side walls...")
    ...
    # Set base thickness (default to 10% of terrain height range)
    if base_thickness is None:
        terrain_height = np.max(all_vertices[:, 2]) - min_z
        base_thickness = max(terrain_height * 0.1, 1.0)  # At least 1 meter
